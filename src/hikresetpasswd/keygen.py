...
支持两种方式 / Supports two methods:
  1. 离线算法：MD5(序列号 + 日期)
     Offline algorithm: MD5(serial number + date)
     适用于 SADP 中显示的设备序列号 + 设备当前日期。
     Uses the device serial number shown in SADP + the current device date.
...
def generate_key_v1(serial: str, reset_date: date) -> str:
    """
    为海康威视设备生成密码重置密钥。
    Generate a password reset key for Hikvision devices.
